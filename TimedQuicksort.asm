...
main:
	...
	getTime($s7)		#get end time time
	sub $s7, $s7, $s6	#calculate total time
	printR($s7)		#print total time
	printW(newline)		#print newline
...
swap:				#swap method
	lw   $t6, 0($t4)	#load value of MRLT
	lw   $t7, 0($t5)	#load value of LRGT
	sw   $t7, 0($t4)	#store value of LRGT at MRLT address
	sw   $t6, 0($t5)	#store value of MRLT at LRGT address
	addi $t5, $t5, 4	#move LRGT to next element which will always be LRGT
